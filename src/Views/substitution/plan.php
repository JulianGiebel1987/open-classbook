...
<div class="page-header">
    ...
    <div class="page-header-actions">
        <a href="/substitution/plan?date=<?= $prevDate ?>" class="btn btn-sm btn-secondary">&laquo; Zurueck</a>
        ...
        <a href="/substitution" class="btn btn-sm btn-secondary">Uebersicht</a>
    </div>
</div>
...
<div class="card sub-publish-bar">
    <?php if ($plan && $plan['is_published']): ?>
        <div class="sub-status sub-status-published">
            <span>Veroeffentlicht am <?= htmlspecialchars(date('d.m.Y H:i', strtotime($plan['published_at'])), ENT_QUOTES, 'UTF-8') ?></span>
    ...
    <?php else: ?>
        <div class="sub-status sub-status-draft">
            <span>Entwurf – noch nicht veroeffentlicht</span>
...
<!-- Abwesende Lehrkraefte -->
<?php if (!empty($absentTeachers)): ?>
<div class="card">
    <div class="card-header">
        <h2>Abwesende Lehrkraefte (<?= count($absentTeachers) ?>)</h2>
    </div>
...
<div class="card">
    <p class="text-muted">Keine abwesenden Lehrkraefte an diesem Tag.</p>
</div>
