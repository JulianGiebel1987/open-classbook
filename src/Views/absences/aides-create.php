<div class="page-header">
    <h1>Schulbegleiter:innen-Abwesenheit eintragen</h1>
</div>

<div class="card">
    <form method="post" action="/absences/aides">
        <?= \OpenClassbook\View::csrfField() ?>

        <div class="form-group">
            <label for="aide_id">Schulbegleiter:in <span aria-hidden="true">*</span><span class="sr-only">(Pflichtfeld)</span></label>
            <select name="aide_id" id="aide_id" class="form-control" required>
                <option value="">- Wählen -</option>
                <?php foreach ($aides as $a): ?>
                    <option value="<?= $a['id'] ?>"><?= htmlspecialchars($a['lastname'] . ', ' . $a['firstname'], ENT_QUOTES, 'UTF-8') ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div class="btn-group">
            ...
            <a href="/absences/aides" class="btn btn-secondary">Abbrechen</a>
        </div>
    </form>
</div>
